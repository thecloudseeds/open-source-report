...
class GitHubRepoExtractor(GitHubAPI):
    ...
    def __init__(self):
        ...
        self.important_filenames = self.config["DB_FILES"] + list(
            self.config["CI_CD"].keys()) + self.config["DOC_FILES"]
    ...
    def get_filenames(self) -> List[str]:
        ...
        while stack:
            ...
            if not data:
                self.logger.warning(
                    "Failed to retrieve contents for: %s", contents_url)
                continue

            self.logger.info("Retrieved %d items for: %s",
                             len(data), self.base_url)
            for item in data:
                ...
                elif item["type"] == "dir" and item["name"].lower() not in self.config["UN_IMPORTANT_DIR"]:
    ...
    def get_ci_cd_tool(self) -> Optional[str]:
        ...
        """
        ci_cd_filenames = set(self.config["CI_CD"].keys())
        ...
        if intersection:
            ...
            return self.config["CI_CD"][next(iter(intersection))]
